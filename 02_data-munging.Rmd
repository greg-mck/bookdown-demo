...
```{r}
unit_visitation <- function(UNITCODE = "ROMO", STARTMONTH = 01, STARTYEAR = 1980, ENDMONTH = 12, ENDYEAR = 2021){

  raw_data <- GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=",UNITCODE,"&startMonth=",STARTMONTH,"&startYear=",STARTYEAR,"&endMonth=",ENDMONTH,"&endYear=",ENDYEAR))
...
output_unit_parks <- parks %>% 
  map(~ unit_visitation(UNITCODE = .))

units <- bind_rows(output_unit_parks)
...
visitation <- visitation %>% mutate(UnitName = ifelse(is.na(UnitName), "Parkwide", UnitName))
...
plotly::ggplotly(ggplot(
  ...
  mapping = aes(
    x = Year,
    y = RecVisitation,
    fill = (reorder(Park, RecVisitation))
  )
) +
  geom_col(position = "stack")+
  labs(title = "Annual Percentage of Total Visitation by Park", y = "Percentage of total visitation")+
  guides(label = FALSE) #how tf do you change the legend title?
)
#
#   ggplot(data=narrow_percent, mapping = aes(x = Year, y = RecVisitation))+
#   geom_col(data=narrow_percent, fill = Park))
...
joined_percent_data <- joined_data %>%
  rename("Park" = "UnitCode.x") %>% 
  group_by(Park, Year) %>%
  summarise(Unit_Rec_Visitors = sum(RecreationVisitors.x), Parkwide_Rec_Visitors = sum(RecreationVisitors.y)) %>% 
  mutate(percent_of_total = (Unit_Rec_Visitors/Parkwide_Rec_Visitors*100)) %>% 
  select(c(Year, Park, percent_of_total))

plotly::ggplotly(ggplot(
  data = joined_percent_data,
  mapping = aes(
    x = Year,
    y = percent_of_total,
    fill = (reorder(Park, percent_of_total))
  )
) +
  geom_col(position = "stack"))
```
...
sumstats <- park_t %>%
  ...
  summarise(
    n = n(),
    mean = mean(percent_of_total),
    sd = sd(percent_of_total),
    se = sd / sqrt(n))
sumstats

```

**GRSM had the highest average visitation during that period (3.5% of total NP visitation) and ACAD had the least average visitation (0.92% of total).**
